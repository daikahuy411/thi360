...
export default function QuestionCategoryTree(props: Props) {
  ...
  const fetchData = () => {
    if (!props.catalogId || props.catalogId == 0) return
    new QuestionCategoryApi().getByCatalog(props.catalogId).then((response: any) => {
      setData([{ id: 0, name: 'Tất cả Danh mục Câu hỏi' }, ...response.data])
    })
  }
