...
const QuestionTable = () => {
  ...
  useEffect(() => {
    searchQuestion()
  }, [questionCatalogId, questionCategoryId, keyword, page, rowsPerPage])
  ...
  const editUrl = id => {
    if (questionCategoryId && questionCategoryId != '0') {
      return `/apps/question-catalog/${questionCatalogId}/questions/${id}/${questionCategoryId}`
    }
          ...
          <Menu keepMounted id='simple-menu' anchorEl={anchorEl} onClose={handleClose} open={Boolean(anchorEl)}>
            {questionTypes.map(item => (
              <MenuItem
                component={Link}
                key={item.id}
                href={`/apps/question-catalog/${questionCatalogId}/questions/add/${item.typeId}/category/${questionCategoryId}`}
              >
                {item.name}
              </MenuItem>
            ))}
