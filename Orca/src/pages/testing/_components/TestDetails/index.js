...
class TestDetails extends React.Component {
  ...
  initialize(examAttempt) {
    examAttempt.userExamAttemptTracking.questionVisiteds =
      examAttempt.userExamAttemptTracking.questionVisiteds || [];
    ...
    this.setState(
      {
        ...
        userAnswers:
          examAttempt.userAnswersJSON !== ""
            ? JSON.parse(examAttempt.userAnswersJSON)
            : {},
  ...
  changeTestGroupSection(section) {
    ...
    var currentQuestionId =
      firstQuetion.questionTypeId == QuestionType.GQ
        ? firstQuetion.children[0].id
        : firstQuetion.id;
  ...
  saveAndNext() {
    ...
    currentQuestionIndex = _.findIndex(
      this.state.examAttempt.testGroup.questionMaps,
      {
    ...
    const nextQuestion =
      this.state.examAttempt.testGroup.questionMaps[currentQuestionIndex];
    this.viewQuestion(
      nextQuestion.id,
  ...
  sectionClassName(sectionId) {
    const normalClassName =
      "b--black-20 gray pointer f6 ba br-pill flex flex-shrink-0 h2 items-center mv2 mr2 ph3";
  ...
  questionClassName(questionId) {
    ...
    if (this.state.mode == 1) {
      if (
        this.state.userExamAttemptTracking.questionAttempteds.indexOf(
          questionId
        ) > -1
      ) {
        return answeredClassName;
      } else if (
        this.state.userExamAttemptTracking.questionVisiteds.indexOf(
          questionId
        ) > -1
      ) {
      ...
      if (
        ...
        if (
          ...
          this.state.userExamAttemptTracking.questionCorrects.indexOf(
            questionId
          ) > -1
        ) {
          return boorkmarkedAndAnsweredClassName;
        ...
        if (
          ...
          this.state.userExamAttemptTracking.questionIncorrects.indexOf(
            questionId
          ) > -1
        ) {
          return boorkmarkedAndNotAnsweredClassName;
  ...
  clearCurrentAttemptedQuestion() {
    ...
    let questionAttempteds = [
      ...this.state.userExamAttemptTracking.questionAttempteds,
    ];

    const currentQuestionIndex = questionAttempteds.indexOf(
  ...
  markQuestion(question, answerId) {
    ...
    let questionIncorrects =
      this.state.userExamAttemptTracking.questionIncorrects;
  ...
  viewQuestion(questionId, parentQuestionId, sectionItemId, sectionId) {
    ...
    var section = this.state.examAttempt.testGroup.sections.filter(
      (x) => x.id == sectionId
    )[0];
    var sectionItem = section.items.filter((x) => x.id == sectionItemId)[0];
    var question = sectionItem.questions.filter(
      (x) => x.id == parentQuestionId
    )[0];

    const questionVisiteds = [
      ...this.state.userExamAttemptTracking.questionVisiteds,
    ];
  ...
  componentDidMount() {
    ...
    // this.setState({ userProfile: authService.getUserProfile() });
  }
                  ...
                  <>
                    {this.state.mode !== 2 && (
                      <div className="flex items-center pa3 flex-grow-1">
                        <span className="darkest-blue flex-l dn">
                          Thời gian còn lại:
                        </span>
                        <span className="f4 fw7 flex-grow-1 justify-center flex darkest-blue">
                          <Countdown
                            ...
                            date={
                              Date.now() +
                              this.state.examAttempt.timeLeftInSeconds * 1000
                            }
                            renderer={Clock}
                          />
                        </span>
                    ...
                    <div className="flex w-100 overflow-auto justify-center-l justify-start ph3">
                      <div className="mt3 flex flex-column">
                        <div className="bg-light-sky-blue flex flex-shrink-0 br--top br3">
                          <div className="flex w4-5 flex-shrink-0 pa3 sky-blue">
                            PHẦN
                          </div>
                          ...
                          <div className="flex flex-column tc w4-5 flex-shrink-0 pa3">
                            ...
                            <span className="f4 fw6 mt3">
                              {
                                this.state.userExamAttemptTracking
                                  .questionSubmitteds.length
                              }
                            </span>
                          </div>
                          <div className="flex flex-column tc w4-5 flex-shrink-0 pa3">
                          ...
                          {this.state.examAttempt.testGroup.sections.map(
                            (section) => (
                              <div
                                ...
                                key={`es-s-${section.id}`}
                              >
                                <div className="flex w4-5 flex-shrink-0 pa3 bg-black-05 gray">
                                  {section.Name}
                                </div>
                  ...
                  <div className="flex">
                    <div className="flex flex-column h-100 w-100 w-80-l br-l bn b--black-20 content-container justify-between-l">
                      <div className="flex mh4-l mh3 flex-shrink-0">
                        <div className="flex-l dn flex-shrink-0 gray items-center f6">
                          PHẦN :
                        </div>
