...
class TestReview extends React.Component {
  ...
  initialize(test) {
    ...
    if (!question) {
      question =
        test.testGroup.sections[0].items[0]
          .questions[0];
    }
  ...
  saveAndNext() {
    ...
    const nextQuestion =
      this.state.test.testGroup.questionMaps[currentQuestionIndex];
  ...
  viewQuestion(questionId, parentQuestionId, sectionItemId, sectionId) {
    ...
    var section = this.state.test.testGroup.sections.filter(
      (x) => x.id == sectionId
    )[0];
