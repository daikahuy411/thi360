          ...
          {isSuccess && (
            <>
              ...
              <Typography sx={{ mb: 4, color: 'text.secondary' }}>
                ...
                {moment(orderInfo?.createdTime).format('DD-MM-YYYY hh:mm:ss')}!
              </Typography>
              <Typography sx={{ mb: 4, color: 'text.secondary' }}>
                Gói được kích hoạt tự động với thời gian sử dụng từ ngày{' '}
        ...
        <Collapse in={true}>
          {!isSuccess && status == 1 && (
            <Alert severity='info'>
              ...
              Gói dịch vụ của bạn đã được đăng ký, Bạn sẽ được chuyển hướng sau{' '}
          ...
          {isSuccess && (
            <Alert severity='success'>
              ...
              Gói dịch vụ của bạn đã được thanh toán thành công, Bạn sẽ được chuyển hướng sau{' '}
              <strong>{formatNumber.add0(countDownValue)}</strong> giây — hoặc <strong>Chuyển hướng ngay</strong>
            </Alert>
          )}
              ...
              <ListItem>
                ...
                <Grid container spacing={5}>
                  <Grid item xs={12} sm={8}>
                    <ListItemText
                      primary={`Gói ${orderInfo?.pricingPlan?.name} dành cho ${orderInfo?.pricingPlan?.typeName}`}
                    />
                    <Box sx={{ display: 'flex' }}>
                      ...
                      >
                        {orderInfo && `${orderInfo.totalMonth} tháng`}
                      </Typography>
                      {orderInfo && orderInfo.promotionCode && (
                        <>
                          Mã giảm giá:{' '}
                          <CustomChip size='small' skin='light' color='success' label={orderInfo.promotionCode} />
                        </>
                      )}
                    </Box>
                  </Grid>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  >
                    ...
                    <Typography variant='body2'>
                      {orderDetailMain?.amount ? formatCurrency(orderDetailMain?.amount, 0) : ''} đ
                    </Typography>
                  </Box>
                  <Box
                    sx={{
                        ...
                        >
                          ...
                          <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                            <Typography variant='body2' sx={{ mr: 2 }}>
                              {item.promotionType === PromotionType.ADDMONTH && (
                                <span>{formatNumber.add0(item.months)} tháng</span>
                              )}
              ...
              <CardContent sx={{ py: theme => `${theme.spacing(3.5)} !important` }}>
                ...
                >
                  ...
                  <Typography sx={{ fontWeight: 600 }} color={'green'}>
                    {formatNumber.add0(orderInfo.totalMonth)} tháng
                  </Typography>
                </Box>
                <Box
                  sx={{
                ...
                >
                  ...
                  <Typography sx={{ fontWeight: 600 }} color={'green'}>
                    {orderInfo ? formatCurrency(orderInfo.totalAmount, 0) : ''} đ
                  </Typography>
                </Box>
              </CardContent>
            </Box>
