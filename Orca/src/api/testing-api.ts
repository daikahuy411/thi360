...
export default class TestingApi extends ApiBase {
  ...
  CreateExamAttempt = (examId: any, examItemId: any, testId: any, mode: any, token: any) => {
    var request = {
      examId: examId,
  ...
  CreateAttemptForCourse = (classId: any, courseId: any, lectureId: any, testId: any, token: any) => {
    var request = {
      classId: classId,
      courseId: courseId,
      lectureId: lectureId,
  ...
  //  Update Exam Attempt
  UpdateExamAttempt = (token: any, userAnswers: any, userExamAttemptTracking: any) => {
    var request = {
      token: token,
  ...
  FinishExamPractice = (token: any, answerVersion: any, userAnswers: any, userAttemptStat: any) => {
    var request = {
      token: token,
  ...
  FinishExamAttempt = (token: any, userAnswers: any, userAnswerMedia: any, userAttemptStat: any) => {
    var request = {
      token: token,
      userAnswers: userAnswers,
  ...
  // Lấy bảng xếp hạng
  GetExamResultStat = (classBlockId: any, limit: any, fromDate: any, toDate: any) => {
    var query = {
  ...
  StartExamAttempt = (token: string, timeToken: string) => {
    return axios.get(this.baseApiUrl + '/StartExamAttempt/' + token + '/' + timeToken)
  }
  ...
  GetExamAttemptHistoryGroupExamByUserId = (query: any) => {
    return axios.post(this.baseApiUrl + '/GetExamAttemptHistoryGroupExamByUserId', query)
  }

  // Lấy danh sách môn thi có lịch sử thi của kỳ thi của người dùng
  GetExamItemAttempHistoryByUserId = (query: any) => {
    return axios.post(this.baseApiUrl + '/GetExamItemAttempHistoryByUserId', query)
  }

  // lấy lịch sử thi của người dùng theo userid, kỳ thi, môn thi, đề thi
  UserExamAttemptHistory = (param: any) => {
    return axios.post(this.baseApiUrl + '/UserExamAttemptHistory', param)
  }
}
